...
Observer::Observer(std::ofstream &out): obs_out( out ) { }


void Observer::operator () (const std::vector<double>& y, double x){
    obs_out << x <<  ", " << y[0] << ", " << y[1] << ", " << constants::R_star * Globals::omega / (2.0 * constants::c) * Lambda(x) << ", " << BetaB (x) + delta (x) << std::endl;
}
...
void odeint(
    ...
    // string path = Globals::out_path + "/" + Globals::RUN_ID + "_theta_data/" + Globals::RUN_ID + "_theta_"+ to_string(Globals::PHI0 * 180 / constants::PI) + ".dat";
    // ofstream plot(path);
    ...
    for (nstp=1;nstp<=MAXSTP;nstp++) {
        ...
        // plot << x <<  ", " << y[0] << ", " << y[1] << ", " << constants::R_star * Globals::omega / (2.0 * constants::c) * Lambda(x) << ", " << BetaB (x) + delta (x) << endl;
