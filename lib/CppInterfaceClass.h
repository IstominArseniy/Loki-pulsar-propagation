...
class CppInterface{
    ...
    std::map<std::string, double> find_ILVPA(double phi, int mode, bool with_absorption=true, bool no_drift=false); // will be exposed to Python
    double find_I(double phi, int mode, bool with_absorption=true);
    double get_R_escape();
    double get_RLC();
    double get_rho();
    std::vector<std::map<std::string, double> > calculate_profile(double phi1, double phi2, double phi_step, int mode, bool with_absorption=true, bool no_drift=false);
