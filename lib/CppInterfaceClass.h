...
class CppInterface{
    ...
    std::map<std::string, double> find_ILVPA_initial(double phi, int mode); // will be exposed to Python
    ...
    std::vector<std::map<std::string, double> > calculate_initial_profile(double phi1, double phi2, double phi_step, int mode);
